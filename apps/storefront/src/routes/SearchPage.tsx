    ...
    <main className="search-page" id="main-content">
      ...
      <div className="search-layout">
        ...
        <section className="search-results" aria-live="polite">
          ...
          {results.length ? <div className="search-product-grid">{results.slice(0, limit).map((product) => <article className="ab-product-card" key={product.id}><Link to={`/products/${product.id}`}><div className="ab-product-card__image"><img alt={product.name} loading="lazy" src={product.image} /></div><h2>{product.name}</h2><p>{product.audience} · {product.category}</p><strong>{formatVnd(product.price)}</strong></Link></article>)}</div> : <div className="empty-state"><Search aria-hidden="true" /><h2>Chưa tìm thấy sản phẩm</h2><p>Thử bỏ bớt bộ lọc hoặc dùng từ khóa khác.</p></div>}
