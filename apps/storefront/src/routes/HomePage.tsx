...
export function HomePage() {
  ...
  return (
    <main id="main-content" className="ab-home">
      <section className="ab-hero">
        ...
        <div className="ab-hero__content"><p>{heroCopy.eyebrow}</p><h1>{heroCopy.title}</h1><span>{heroCopy.description}</span><div className="ab-actions"><Link className="ab-button ab-button--light" to="/women">Khám phá đồ nữ</Link><Link className="ab-button ab-button--light" to="/men">Khám phá đồ nam</Link></div></div>
      </section>
      ...
      <section className="ab-section ab-products">
        ...
        <div className="ab-product-grid">
          {products.map((product, index) => (
            <article className="ab-product-card" key={product.name}>
      ...
      <section className="ab-campaign">
        <img alt="Dép HORIZ trên bãi cát" src="/prototype/campaign-sand.jpg" />
        <div className="ab-campaign__content">
      ...
      <section className="ab-story-grid">
        <article className="ab-story-card">
      ...
      <section className="ab-benefits">
        {benefits.map(({ icon: Icon, title, copy }) => <article key={title}><Icon aria-hidden="true" /><h3>{title}</h3><p>{copy}</p></article>)}
      </section>
    </main>
  )
}
